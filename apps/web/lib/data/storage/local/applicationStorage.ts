...
function normalizeApplication(value: unknown, fallback?: Application): Application {
  ...
  return {
    ...
    externalJobId: stringValue(item.externalJobId, fallback?.externalJobId ?? ""),
    capturedAt: stringValue(item.capturedAt, fallback?.capturedAt ?? ""),
    nextAction: stringValue(item.nextAction, fallback?.nextAction ?? ""),
    nextActionDueAt: stringValue(item.nextActionDueAt, fallback?.nextActionDueAt ?? ""),
    nextEventType: stringValue(item.nextEventType, fallback?.nextEventType ?? ""),
