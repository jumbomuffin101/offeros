            ...
            <aside className="min-w-0 space-y-5">
              ...
              <section className="rounded-xl border border-indigo-300/15 bg-indigo-300/[0.045] p-4">
                ...
                {prepPlan ? <div className="mt-4 space-y-4">
                  ...
                  <div className="rounded-lg border border-white/10 bg-white/[0.025] p-3"><div className="flex items-center justify-between gap-3"><div><p className="text-xs font-medium uppercase text-slate-500">Behavioral stories</p><p className="mt-1 text-sm text-slate-300">{prepPlan.plan.behavioral.recommended_story_ids?.length ?? 0} matching stories · {prepPlan.plan.behavioral.missing_story_categories?.length ?? 0} missing competencies</p></div><Link className="rounded-lg border border-indigo-300/20 bg-indigo-300/10 px-3 py-2 text-sm font-medium text-indigo-100 transition hover:bg-indigo-300/15" href={prepPlan.plan.behavioral.coach_route ?? `/prep?tab=behavioral&application=${application.id}`}>Practice now</Link></div><div className="mt-2 flex flex-wrap gap-1.5">{prepPlan.plan.behavioral.focus_areas?.map((item) => <Badge key={item.category} tone={item.story_ids?.length ? "green" : "amber"}>{item.category}: {item.story_ids?.length ? "story ready" : "missing"}</Badge>)}</div></div>
