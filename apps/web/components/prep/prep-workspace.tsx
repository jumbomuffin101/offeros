...
export function PrepWorkspace() {
  ...
  const [behavioralApplicationId, setBehavioralApplicationId] = useState<string | undefined>();
  const [activeTab, setActiveTab] = useState<"overview" | "behavioral" | "mock-interviews">("overview");

  useEffect(() => {
    window.queueMicrotask(() => {
      const requestedTab = new URLSearchParams(window.location.search).get("tab");
      if (requestedTab === "mock-interviews" || requestedTab === "behavioral") setActiveTab(requestedTab);
      setBehavioralApplicationId(new URLSearchParams(window.location.search).get("application") ?? undefined);
  ...
  useEffect(() => {
    ...
    if (storyId) window.queueMicrotask(() => setBehavioralOpen(data.behavioralQuestions.find((item) => item.id === storyId) ?? null));
  }, [data]);
  useEffect(() => {
  ...
  const navigation = <div className="flex w-fit items-center gap-1 rounded-lg border border-slate-700/45 bg-slate-900/25 p-1">
    ...
    <button className={`rounded-md px-3 py-2 text-sm font-medium transition ${activeTab === "behavioral" ? "bg-indigo-400/15 text-indigo-100" : "text-slate-400 hover:text-slate-200"}`} onClick={() => setActiveTab("behavioral")} type="button">Behavioral Coach</button>
  ...
  const behavioralDrawer = <BehavioralAnswerDrawer question={behavioralOpen} onClose={() => setBehavioralOpen(null)} onSave={saveBehavioral} onEvaluate={(storyId, competencyFocus) => prepData.evaluateBehavioral(storyId, { competencyFocus, applicationId: behavioralApplicationId })} onHistory={prepData.listBehavioralEvaluations} onPractice={(input) => prepData.practiceBehavioral({ ...input, applicationId: behavioralApplicationId })} />;
  if (activeTab === "behavioral") return <div className="space-y-5">{navigation}<BehavioralPracticeCard questions={data.behavioralQuestions} loadPortfolio={prepData.behavioralPortfolio} onEdit={(question) => setBehavioralOpen(question)} onStatus={changeBehavioralStatus} />{behavioralDrawer}<Toast message={toast} /></div>;
