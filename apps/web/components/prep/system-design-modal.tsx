...
import { Field, ModalShell, Textarea } from "@/components/prep/coding-problem-modal";
...
function SystemDesignContent({ designPrompt, onClose, onSubmit }: { designPrompt: SystemDesignPrompt | null; onClose: () => void; onSubmit: (payload: SystemDesignPayload) => void }) {
  ...
  return <ModalShell title={designPrompt ? "Edit system design prompt" : "Add system design prompt"} onClose={onClose} onSave={submit} saveLabel={designPrompt ? "Save changes" : "Add prompt"}>{error ? <div className="rounded-xl border border-rose-300/20 bg-rose-300/10 px-4 py-3 text-sm text-rose-100">{error}</div> : null}<div className="grid gap-4 sm:grid-cols-2"><Field label="Title" required><Input value={form.title} onChange={(event) => update("title", event.target.value)} /></Field><div><span className="mb-1.5 block text-xs font-medium text-slate-500">Status</span><PrepStatusSelect label="System design prompt status" value={form.status} onChange={(status: PrepStatus) => update("status", status)} /></div><Field className="sm:col-span-2" label="Prompt" required><Textarea value={form.prompt} onChange={(value) => update("prompt", value)} /></Field><Field className="sm:col-span-2" label="Concepts"><Input placeholder="Caching, queues, idempotency" value={concepts} onChange={(event) => setConcepts(event.target.value)} /></Field><Field className="sm:col-span-2" label="Notes"><Textarea rows={6} value={form.notes} onChange={(value) => update("notes", value)} /></Field></div></ModalShell>;
}
