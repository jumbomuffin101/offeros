...
Behavioral Coach uses migration `20260805_0021`. Its isolated verification sequence is:
...
The existing `behavioral_questions` table remains the story source of truth. Evaluation history and focused practice are stored in `behavioral_story_evaluations` and `behavioral_practice_sessions`; live evaluation reuses the configured OpenRouter provider and deterministic guidance remains available without AI.
